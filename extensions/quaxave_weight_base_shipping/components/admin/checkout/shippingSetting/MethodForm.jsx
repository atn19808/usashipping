...
function MethodForm({ saveMethodApi, closeModal, getZones, method }) {
  const [type, setType] = React.useState(() => {
    if (method?.calculateApi) {
      return 'api';
    } else if (method?.priceBasedCost) {
      return 'price_based_rate';
    } else if (method?.weightBasedCost) {
      return 'weight_based_rate';
    } else if (method?.weightBasedRate) {
      return 'weight_based_rate_v2';
    }
    return 'flat_rate';
  });
        ...
        <Card.Session title="Setup shipping cost">
          <Radio
            ...
            options={[
              ...
              { text: 'Weight based rate V2', value: 'weight_based_rate_v2' },
...
MethodForm.propTypes = {
  ...
  method: PropTypes.shape({
    ...
    weightBasedRate: PropTypes.shape({
      value: PropTypes.number
    }),
    conditionType: PropTypes.string,
    min: PropTypes.number,
